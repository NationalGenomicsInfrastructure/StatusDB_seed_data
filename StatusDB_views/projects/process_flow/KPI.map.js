function(doc) {
  ...
  // NJ 2013-10-22
  // Added close date. This is date is emitted further down in the code.
  // Also added to Reduce Function ---->
...
      // key "sequencing_finish_date" 
      // NJ 2013-11-06: corrected code to get the latest seq finished date
      var final_sequence_date = "0000-00-00"
      for (sample_run_metrics in LP["sample_run_metrics"]) {
        //final_sequence_date = LP["sample_run_metrics"][sample_run_metrics]["sequencing_finish_date"]
        var tmp_fin_seq = LP["sample_run_metrics"][sample_run_metrics]["sequencing_finish_date"]
        if(tmp_fin_seq > final_sequence_date) { final_sequence_date = tmp_fin_seq; }
      }
      ...
      emit([sequencing_platform, doc["project_name"], doc["project_id"], sample, library_prep], KPI)
    }
    ...
    if (!("library_prep" in doc["samples"][sample])) {
      ...
      emit([sequencing_platform, doc["project_name"], doc["project_id"], sample, null], KPI)
    }
  }
  ...
  if (!("samples" in doc)) {
    ...
    emit([sequencing_platform, doc["project_name"], doc["project_id"], null, null], KPI)
  }
}