function(doc) {
  ...
  for (sample in doc["samples"]){
    ...
    for (library_prep in doc["samples"][sample]["library_prep"]) {
      ...
      if(LP["prep_status"] == "FAILED") {
        final_validation = "0000-00-00";
      } else {
      ...
      emit([application, doc["project_name"], sample, library_prep], KPI)
    }
    ...
    if (!("library_prep" in doc["samples"][sample])) {
      ...
      emit([application, doc["project_name"], sample, null], KPI)
    }
  }
  ...
  if (!("samples" in doc)) {
    ...
    emit([application, doc["project_name"], null, null], KPI)
  }
}